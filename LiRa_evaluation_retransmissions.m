...
load WARP_MEASURES/OLD_DATA_WITH_RETRANSMISSIONS/FORMATTED_DATA/80211_VLC_LU_performance_metrics.mat
res_del_80211 = response_delay;
wifi_tput_deg_80211 = legacy_wifi_tput_deg;

%LiRa - Night
load WARP_MEASURES/OLD_DATA_WITH_RETRANSMISSIONS/FORMATTED_DATA/LiRa_performance_metrics.mat
res_del_lira = response_delay;
wifi_tput_deg_lira = legacy_wifi_tput_deg;

%LiRa - 1 Gbps
load WARP_MEASURES/OLD_DATA_WITH_RETRANSMISSIONS/FORMATTED_DATA/LiRa_1Gbps_performance_metrics.mat;
res_del_lira_1Gbps = response_delay;
wifi_tput_deg_lira_1Gbps = legacy_wifi_tput_deg;

%LiRa - 10 Mbps
load WARP_MEASURES/OLD_DATA_WITH_RETRANSMISSIONS/FORMATTED_DATA/LiRa_10Mbps_performance_metrics.mat;
...
figure;
colormap inferno;
y = 0.001*[median(res_del_lira(1,1,:,1,1,2,1),3) median(res_del_lira(5,1,:,1,1,2,1),3) median(res_del_lira(10,1,:,1,1,2,1),3);
    median(res_del_lira(1,1,:,2,1,2,1),3) median(res_del_lira(5,1,:,2,1,2,1),3) median(res_del_lira(10,1,:,2,1,2,1),3);
    median(res_del_lira(1,1,:,3,1,2,1),3) median(res_del_lira(5,1,:,3,1,2,1),3) median(res_del_lira(10,1,:,3,1,2,1),3);
    ];
bar(y)
grid on;
...
y = 0.001*[median(res_del_lira(10,1,:,trigg_ind,1,1,1),3)% median(res_del_lira(10,1,:,trigg_ind,2,1,1),3) median(res_del_lira(10,1,:,trigg_ind,3,1,1),3);
    ...
    median(res_del_lira(10,2,:,trigg_ind,1,1,1),3)% median(res_del_lira(10,2,:,trigg_ind,2,1,1),3) median(res_del_lira(10,2,:,trigg_ind,3,1,1),3);
    median(res_del_lira(10,2,:,trigg_ind,1,3,1),3)% median(res_del_lira(10,2,:,trigg_ind,2,3,1),3) median(res_del_lira(10,2,:,trigg_ind,3,3,1),3);
    ];
bar(y)
grid on;
xlabel('LEGACY RF TRAFFIC FLOWS - MCS');
ylabel('RESPONSE DELAY (ms)');
set(gca,'XTickLabel',{'1 FLOW - 6 Mbps', '1 FLOW - 54 Mbps', '2 FLOWS - 6 Mbps', '2 FLOWS - 54 Mbps'})
legend(legend_wifi);
set(gca,'FontSize',24,'fontWeight','bold');
set(findall(gcf,'type','text'),'FontSize',24,'fontWeight','bold');
...
figure;
colormap inferno;
y = [median(wifi_tput_deg_lira(1,1,:,1,1,2),3) median(wifi_tput_deg_lira(5,1,:,1,1,2),3) median(wifi_tput_deg_lira(10,1,:,1,1,2),3);
    median(wifi_tput_deg_lira(1,1,:,2,1,2),3) median(wifi_tput_deg_lira(5,1,:,2,1,2),3) median(wifi_tput_deg_lira(10,1,:,2,1,2),3);
    median(wifi_tput_deg_lira(1,1,:,3,1,2),3) median(wifi_tput_deg_lira(5,1,:,3,1,2),3) median(wifi_tput_deg_lira(10,1,:,3,1,2),3);
];
bar(y)
grid on;
xlabel('FEEDBACK TRIGGER TIME (ms)');
...
% 
% 
f = [median(wifi_tput_deg_lira(1,1,:,1,1,2),3) median(wifi_tput_deg_lira(1,1,:,2,1,2),3) median(wifi_tput_deg_lira(1,1,:,3,1,2),3) median(wifi_tput_deg_80211(1,:,1,2),2);
    median(wifi_tput_deg_lira(1,2,:,1,1,2),3) median(wifi_tput_deg_lira(1,2,:,2,1,2),3) median(wifi_tput_deg_lira(1,2,:,3,1,2),3) median(wifi_tput_deg_80211(2,:,1,2),2);
    ];
figure
bar(f)
