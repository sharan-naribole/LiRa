function [res_del_vec, wifi_tput_deg, deg_extra] = compute_metrics_LiRa(client_rates, trigg_time, tx_offset, payload_offset)
...
while(interval_count <=min(Ntrigg_intervals,Ntrigg_intervals_max) && interval_iter <=Ntrigg_intervals)
    ...
    LiRa_time = LiRa_time + PIFS + t_CTS + SIFS + Nclients*ack_tx_time;
    total_time = total_time + PIFS + t_CTS + SIFS + Nclients*ack_tx_time + trigg_time + tx_offset(interval_iter);
...
wifi_tput_deg = 100*LiRa_time/total_time;
